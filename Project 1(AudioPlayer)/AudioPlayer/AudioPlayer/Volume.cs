...
namespace AudioPlayer
{
    public partial class Volume : UserControl
    {
        ...
        private void Volume_Paint(object sender, PaintEventArgs e)
        {
            ...
            for (int i = 0; i < (pb_value * ClientSize.Width / 85 - pb_value) / gap; i++)
            {
            ...
            if (pb_value >= 30)
            {
                Image left_img = Properties.Resources.mid_img;
                e.Graphics.DrawImage(left_img, 5, 0, ClientSize.Height, ClientSize.Height);
            }
            if (pb_value <= Min)
            {
            ...
            if (pb_value >= 70)
            {
                Image left_img = Properties.Resources.high_img;
                e.Graphics.DrawImage(left_img, 5, 0, ClientSize.Height, ClientSize.Height);
            }
        }
